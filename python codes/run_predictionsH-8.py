...
def main():
    ...
    dataset_valid = generate_pipeline_training(root_folder, sensor_index, batch_size=1)
    ...
    if sensor_index == 1:
        ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-17"
    elif sensor_index == 2:
        ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-13"
    elif sensor_index == 3:
        ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-12"
    checkpoint.restore(ckpt_file).expect_partial()

    n_samp = 4000
    x_target = np.zeros((n_samp, nx,  1, nz, 1), np.float32)
    ...
    #savemat('matlabmatrix.mat', data) # save this in other code with smaller files
...
class GAN3D(object):
    ...
    def architecture01(self):
        ...
        #inputs = keras.Input(shape=(int(self.nx/self.reduction), 1, int(self.nz/self.reduction), self.input_channels), name='wall-input')
        inputs = keras.Input(shape=(self.nx, 1, self.nz, self.input_channels), name='wall-input')

        reduc = layers.AveragePooling3D(pool_size=(8, 1, 8), strides=None, padding='same', data_format='channels_last')(inputs)
    
        conv_1 = layers.Conv3D(filters=64, kernel_size=9, strides=1, activation='linear', data_format='channels_last', padding='same')(reduc)

        ups_xz = layers.UpSampling3D(size=(8, 1, 8), data_format='channels_last')(conv_1)

        prelu_1 = layers.PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=[1,2,3])(ups_xz)
...
def generate_pipeline_training(root_folder, sensor_index, validation_split=1, shuffle_buffer=200, batch_size=4, n_prefetch=4):
    ...
    dataset_valid = tfr_files_val_ds.map(lambda x: tf_parser(x, root_folder, sensor_index), num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
@tf.function
def tf_parser(rec, root_folder, sensor_index):
    ...
    U_mean = np.expand_dims(np.load(filename)['U_mean'], axis=-1)[:,0:64,:,:]
    V_mean = np.expand_dims(np.load(filename)['V_mean'], axis=-1)[:,0:64,:,:]
    W_mean = np.expand_dims(np.load(filename)['W_mean'], axis=-1)[:,0:64,:,:]
    ...
    U_std = np.expand_dims(np.load(filename)['U_std'], axis=-1)[:,0:64,:,:]
    V_std = np.expand_dims(np.load(filename)['V_std'], axis=-1)[:,0:64,:,:]
    W_std = np.expand_dims(np.load(filename)['W_std'], axis=-1)[:,0:64,:,:]
    ...
    if sensor_index == 1:
        ...
        wall = (tf.reshape(parsed_rec['raw_tx'], (nx, 1, nz, 1)) - TBX_mean) / TBX_std
    elif sensor_index == 3:
        wall = (tf.reshape(parsed_rec['raw_tz'], (nx, 1, nz, 1)) - TBZ_mean) / TBZ_std
...
if __name__ == '__main__':
    ...
    learning_rate = 1e-4
    sensor_index = 3
    model_name = 'architecture-H-8-' + str(sensor_index)  #change model name (sensor_index)
    root_folder = '/    /'
