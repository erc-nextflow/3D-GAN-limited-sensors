...
def main():
    ...
    dataset_valid = generate_pipeline_training(root_folder, err, batch_size=1)
    ...
    ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-16" #E4-1: 16    E4-2: 5    E4-3:18     1-err1 ->18   1-err3  ->18
...
def generate_pipeline_training(root_folder, err, validation_split=1, shuffle_buffer=200, batch_size=4, n_prefetch=4):
    ...
    dataset_valid = tfr_files_val_ds.map(lambda x: tf_parser(x, root_folder, err), num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
@tf.function
def tf_parser(rec, root_folder, err):
    ...
    wall = tf.add((tf.reshape(parsed_rec['raw_p'], (nx, 1, nz, 1)) - PB_mean) / PB_std , tf.random.normal([nx, 1, nz, 1], stddev = err))
    wall = tf.concat((wall, tf.add((tf.reshape(parsed_rec['raw_tx'], (nx, 1, nz, 1)) - TBX_mean) / TBX_std , tf.random.normal([nx, 1, nz, 1], stddev = err))), -1)
    wall = tf.concat((wall, tf.add((tf.reshape(parsed_rec['raw_tz'], (nx, 1, nz, 1)) - TBZ_mean) / TBZ_std , tf.random.normal([nx, 1, nz, 1], stddev = err))), -1)
...
if __name__ == '__main__':
    ...
    LZ = np.pi / 2
    learning_rate = 1e-4
    err = 8 * 0.03  # noise level for testing
    # model name: 'architecture-E04-1', 'architecture-E04-1-err1', 'architecture-E04-1-err3'
    model_name = 'architecture-E04-1-err3'  # change model name, architecture line 32, checkpoint line 51
    root_folder = '/    /'
