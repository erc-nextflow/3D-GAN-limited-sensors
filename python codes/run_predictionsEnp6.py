...
def main():
    ...
    ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-18" #E6-1: 18
...
class GAN3D(object):
    ...
    def architecture01(self):
        ...
        reduc = layers.AveragePooling3D(pool_size=(8, 1, 2), strides=None, padding='same', data_format='channels_last')(inputs)
    
        conv_1 = layers.Conv3D(filters=64, kernel_size=9, strides=1, activation='linear', data_format='channels_last', padding='same')(reduc)

        ups_xz = layers.UpSampling3D(size=(8, 1, 2), data_format='channels_last')(conv_1)
