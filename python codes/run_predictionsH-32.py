...
def main():
    ...
    if sensor_index == 1:
        ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-16"
    elif sensor_index == 2:
        ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-19"
    elif sensor_index == 3:
        ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-16"
...
class GAN3D(object):
    ...
    def architecture01(self):
        ...
        reduc = layers.AveragePooling3D(pool_size=(2, 1, 2), strides=None, padding='same', data_format='channels_last')(inputs)
    
        conv_1 = layers.Conv3D(filters=64, kernel_size=9, strides=1, activation='linear', data_format='channels_last', padding='same')(reduc)

        ups_xz = layers.UpSampling3D(size=(2, 1, 2), data_format='channels_last')(conv_1)
...
if __name__ == '__main__':
    ...
    learning_rate = 1e-4
    sensor_index = 3
    model_name = 'architecture-H-32-' + str(sensor_index)  #change model name (sensor_index)
    root_folder = '/    /'
